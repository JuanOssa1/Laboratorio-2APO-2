...
public class Holding implements Serializable {
	...
	public static String FLATPETS = "data//PetsToSerialize.txt";
		...
		public String addClub(String clu1, String clu2 ,String clu3, String clu4) {
			String msg = "Todo Perfecto"; 
			try{
				clubs.add(new Club(clu1,clu2,clu3,clu4));
		...
		*/
		public void organizeWithId(){
			...
			for (int i = 1; i<clubs.size(); i++){
				for(int j = i; j>0  ; j--){
					if(clubs.get(j-1).compareClubWithId(clubs.get(j)) > 0) {
						Club tmp = clubs.get(j);
						clubs.set(j, clubs.get(j-1));
						clubs.set(j-1, tmp);
					}else if(clubs.get(j-1).compareClubWithId(clubs.get(j)) < 0) {
						Club tmp = clubs.get(j-1);
						clubs.set(j, tmp);
		...
		public void organizeWithName(){
			...
			for (int i = 1; i<clubs.size(); i++){
				for(int j = i; j>0  ; j--){
					if(clubs.get(j-1).compareClubWithName(clubs.get(j)) > 0) {
						Club tmp = clubs.get(j);
						clubs.set(j, clubs.get(j-1));
						clubs.set(j-1, tmp);
					}else if(clubs.get(j-1).compareClubWithName(clubs.get(j)) < 0) {
						Club tmp = clubs.get(j-1);
						clubs.set(j, tmp);
						clubs.set(j-1, clubs.get(j));
					}
		...
		public void organizeWithDate(){
			...
			for (int i = 1; i<clubs.size(); i++){
				for(int j = i; j>0  ; j--){
					if(clubs.get(j-1).compareClubWithDateOfCreation(clubs.get(j)) > 0) {
						Club tmp = clubs.get(j);
						clubs.set(j, clubs.get(j-1));
						clubs.set(j-1, tmp);
					}else if(clubs.get(j-1).compareClubWithDateOfCreation(clubs.get(j)) < 0) {
						Club tmp = clubs.get(j-1);
						clubs.set(j, tmp);
						clubs.set(j-1, clubs.get(j));
					}
...
///////////////////////////////////////////////////////////METODOS QUE NO ESTAN EN DIAGRAMA//////////////////////////////////////////////////////////////////////////
		...
		public String searchClubToDeleteByAnOwnerName(String clubId, String ownerId) {
			String msg = "";
			for(int i = 0; i<clubs.size(); i++) {
				if(clubs.get(i).getId().equals(clubId)) {
					try {
						msg =clubs.get(i).searchOwnerToDeleteWithName(ownerId); 
					} catch (NotFoundedAnythingException e) {
						System.out.println("No encontre datos");
					}
				}
		...
		public String OrganizeOwnersById() {
			String msg = ""; 
			for (int i = 0; i < clubs.size(); i++) {

					clubs.get(i).organizeWithId();
				
			}
			return msg;
		}
		public String OrganizeOwnersByName() {
			String msg = "";
			for (int i = 0; i < clubs.size(); i++) {
			
					clubs.get(i).organizeWithName();
				
			}
			return msg;
		}
		public String OrganizeOwnersByLastName() {
			String msg = "";
			for (int i = 0; i < clubs.size(); i++) {
				
					clubs.get(i).organizeWithLastName();
				
			} 
			return msg;
		}
		public String OrganizeOwnersBybornDate() {
			String msg = "";
			for (int i = 0; i < clubs.size(); i++) {
				
					clubs.get(i).organizeWithDate();
